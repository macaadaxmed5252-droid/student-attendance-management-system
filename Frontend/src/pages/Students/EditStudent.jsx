...
const EditStudent = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        setError('Failed to fetch the target profile record.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Validation system error structural collision occurred.');
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-100 shadow-sm rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input type="text" required value={formData.className} onChange={(e) => setFormData({ ...formData, className: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-indigo-600 transition text-slate-800" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
